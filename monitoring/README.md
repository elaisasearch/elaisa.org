# Elaisa Monitoring Stack

While the application is running in our swarm, we want to see some metrics to check if everything is working the right way. For this task you could use the very helpful [Grafana Starter Kit](https://github.com/stefanprodan/swarmprom) by Stefan Prodan on GitHub. I customized the `docker-compose.traefik.yml` file to include the stack in my traefik environment.

## Install with Traefik

If you have a Docker Swarm cluster with a global Traefik set up, you can deploy Swarmprom integrated with that global Traefik proxy.

This way, each Swarmprom service will have its own domain.


You have to define the following subdomains in your DNS setup for your domain:

* `grafana.elaisa.org`
* `alertmanager.elaisa.org`
* `unsee.elaisa.org`
* `prometheus.elaisa.org`

**Note**: You can also use a subdomain, like `swarmprom.elaisa.org`. Just make sure that the subdomains point to (at least one of) your cluster IPs. Or set up a wildcard subdomain (`*`).
...
The Swarmprom Grafana is preconfigured with two dashboards and Prometheus as the default data source:
